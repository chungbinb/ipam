    ...
    switch ($requestMethod) {
        case 'GET':
            ...
            if ($requestUri === '/api/auth/check') {
                ...
                $controller->getAsset();
            } elseif ($requestUri === '/ip-segments') {
                $authMiddleware->checkAuth(); // 新增验证
                $ipSegmentController->getList();
            } elseif ($requestUri === '/ip-addresses') {
                $authMiddleware->checkAuth(); // 新增验证
                $ipAddressController->getList();
            } elseif ($requestUri === '/ping-ip') {
                $authMiddleware->checkAuth(); // 新增验证
        ...
        case 'POST':
            ...
            } elseif ($requestUri === '/ping-ip-batch') {
                $authMiddleware->checkAuth(); // 新增验证
        ...
        case 'PUT':
            // 所有PUT请求都需要验证
            $authMiddleware->checkAuth();
            if (preg_match('/\/ip-segments\/(\d+)/', $requestUri, $matches)) {
                $ipSegmentController->update($matches[1]);
            } elseif (preg_match('/\/ip-addresses\/(\d+)/', $requestUri, $matches)) {
